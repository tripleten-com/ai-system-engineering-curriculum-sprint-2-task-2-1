...
Purpose:           Validate Task 2.1 answers, the published query set, and change paths.
...
QUERIES_PATH = "infra/corpus/queries.jsonl"
...
def main(root: Path | None = None, *, changed_paths: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    print("Task 2.1 answer verification passed.")
    return 0
...
def validate_submission(
    ...
    *,
    ...
    root = submission_path.parent if task_root is None else task_root
    published = _published_query_ids(root / QUERIES_PATH)
    for field in ("success_query_id", "miss_query_id"):
        if answers[field] not in published:
            raise SubmissionError(
                f"answers.{field} is not a published query identifier; "
                f"use one printed by `poe baseline`"
            )
    if answers["success_query_id"] == answers["miss_query_id"]:
        raise SubmissionError(
            "answers.success_query_id and answers.miss_query_id must name different queries"
        )


def _published_query_ids(queries_path: Path) -> set[str]:
    """Return the identifiers in the published golden query set.

    The query set is the single source of truth. The answer schema constrains
    only the identifier shape, so this check is what keeps a typo or an
    invented identifier from passing structural validation.
    """
    try:
        text = queries_path.read_text(encoding="utf-8")
    except OSError as exc:
        raise SubmissionError(f"published query set is unreadable at {QUERIES_PATH}") from exc
    ...
    for number, line in enumerate(text.splitlines(), start=1):
        ...
        try:
            ...
            raise SubmissionError(f"{QUERIES_PATH} line {number} is not valid JSON") from exc
        identifiers.add(str(record["query_id"]))
    if not identifiers:
        raise SubmissionError(f"{QUERIES_PATH} contains no queries")
    return identifiers
